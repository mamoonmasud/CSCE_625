...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        #Iterating over Living Ghost Position Distributions 
        
        for beliefs_x in livingGhostPositionDistributions:
            ghost_pos = None
            ghost_prob = None
            # Finding the most likely position of the ghost
            for x_pos , x_prob in beliefs_x.items():
                if ghost_prob ==None or x_prob > ghost_prob:
                    ghost_prob = x_prob
                    ghost_pos = x_pos
            #Iterating over all the legal actions
            for action in legal:
                next_pos = Actions.getSuccessor(pacmanPosition, action)
                dist = self.distancer.getDistance(next_pos, ghost_pos)

                if act_x ==None or dist < dist_x :
                    dist_x = dist
                    act_x = action
        return act_x